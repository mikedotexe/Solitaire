...
public class SSmartPlayer extends SPlayer {
    ...
    public void play(){
            ...
            do{
                ...
                for (i=6;i>=0;i--){
                    ...
                    for (int j=0;j<7;j++){
                        ...
                        if (canMove){
                            // Found a match, so move it
                           //System.out.println("db.getting in block L"+ breakme);
                                // AI Rule: Do not move single card on pile if there isn't a KING
                                // to take its place (only for Smart Player)
                                if (game.pile[i].up.size() == 1 && game.pile[i].down.size() == 0){
                                    if (kingsPresent() == false){
                                        // there aren't any kings, so move on to next comparison
                                        canMove = false;
                                        continue;
                                    }
                                }
        ...
        try{
            // Create file for results
             ...
             FileWriter fstream = new FileWriter("SmartPlayer_output.txt",true);
    ...
    public boolean kingsPresent(){
        // One of our AI rules is to never play a card from a Tableau pile 
        // if its the only one in the pile, and there are no Kings to immediately take it's place.
        // So this will only apply to 4) above
        boolean kingHere = false;
        // Check reserve
        if (game.reserve.up.size() > 0) {
            // make sure there are cards up, otherwise it'll throw an error
            if (game.reserve.up.peek().rank() == SCard.Rank.KING) {
                kingHere = true;
            }
        }
        // Check all tableau piles
        for (int i=0;i<7;i++){
            if (game.pile[i].up.size() > 0) {
                if (game.pile[i].up.peek().rank() == SCard.Rank.KING) {
                    kingHere = true;
                }
            }
        }
